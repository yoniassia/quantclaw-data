...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'scan_universe';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.mean_reversion_scanner as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
