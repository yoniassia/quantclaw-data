...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_order_status';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.fix_protocol_gateway as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
