...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'detect_ticker_spike';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.social_sentiment_spikes as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
