...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'check_equity_ownership_threshold';

  try {
    const { execSync } = await import('child_process');
    const args = ticker ? `'${ticker.replace(/'/g, "\\'")}'` : '';
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.position_limit as m; import json; print(json.dumps(m.${action}(${args})))"`;
