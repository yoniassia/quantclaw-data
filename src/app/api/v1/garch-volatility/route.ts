...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'egarch_fit';

  try {
    const { execSync } = await import('child_process');
    const query = searchParams.get('query') || '';
    const args = query ? `'${query.replace(/'/g, "\\'")}'` : '';
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.garch_volatility as m; import json; print(json.dumps(m.${action}(${args})))"`;
