...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_realtime_bid_ask';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.transaction_cost as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
