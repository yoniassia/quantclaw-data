...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_historical_reconstitution_stats';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.index_reconstitution_tracker as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
