...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'calculate_dividend_growth_rate';

  try {
    const { execSync } = await import('child_process');
    const args = ticker ? `'${ticker.replace(/'/g, "\\'")}'` : '';
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.dividend_sustainability as m; import json; print(json.dumps(m.${action}(${args})))"`;
