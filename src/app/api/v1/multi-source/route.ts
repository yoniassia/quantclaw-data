...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_crypto_price_from_coingecko';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.multi_source as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
