...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'build_risk_parity_portfolio';

  try {
    const { execSync } = await import('child_process');
    const args = ticker ? `'${ticker.replace(/'/g, "\\'")}'` : '';
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.risk_parity_portfolio as m; import json; print(json.dumps(m.${action}(${args})))"`;
