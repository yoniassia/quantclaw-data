...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_all_futures';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.agricultural_commodities as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
