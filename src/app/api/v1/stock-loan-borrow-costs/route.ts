...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_borrow_cost_analysis';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.stock_loan_borrow_costs as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
