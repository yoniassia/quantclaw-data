...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'find_depth_walls';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.level2_orderbook as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
