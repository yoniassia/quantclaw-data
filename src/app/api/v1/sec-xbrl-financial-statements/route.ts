...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_cik_from_ticker';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.sec_xbrl_financial_statements as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
