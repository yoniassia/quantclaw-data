...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'discover_signals';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.signal_discovery_engine as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
