...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_baltic_dry_index';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.satellite_proxies as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
