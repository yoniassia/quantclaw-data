...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_float_analysis';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.share_float_and_ownership_structure as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
