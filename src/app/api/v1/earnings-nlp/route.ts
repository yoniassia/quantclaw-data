...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'detect_question_dodging';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.earnings_nlp as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
