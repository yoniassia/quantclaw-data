...
export async function GET(request: NextRequest) {
  ...
  const action = searchParams.get('action') || 'get_consensus_targets';

  try {
    const { execSync } = await import('child_process');
    const cmd = `cd /home/quant/apps/quantclaw-data && python3 -c "import modules.analyst_target_price as m; import json; print(json.dumps(m.${action}('${ticker}')))"`;
